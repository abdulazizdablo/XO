...
class ReviewService
{
    public function getAllReviews($filter_data)
    {
    ...
    {
        $reviews = Review::where('user_id', $user_id)->paginate();
    ...
    public function getReview($review_id) : Review
    {
    ...
    public function createReview($rating ,$comment, $user, int $product_id, $page_size)
    {
...
'percentage' => (string) round($item->count('rating') * 100 / $product->reviews_count,1) ];
		...
		//dd($percentages);
       /* $ratings = Review::where('product_id', $product->id)->groupBy('rating')
     ->selectRaw('rating, count(*) as count, CAST((count(*) * 100.0 / (SELECT COUNT(*) FROM reviews WHERE product_id =?)) AS DECIMAL(5,2)) as percentage', [$product->id])
    ...
    // Example condition: Delete models where 'status' is 'inactive'
    return $model->id == $user_review?->id;
});
	
		
		
        return ['reviews_count'=> $reviews_count,
                'avg_rating'=> $avg_rating,
                'rating'=>$percentages, 

                'user_review' =>  $user_review,
                'reviews'=> /*$product->reviews()->with('user:id,first_name,last_name')->paginate($page_size)*/$product_reviews->values()];
                
        return $review;
    }

    public function updateReview($validated_data, $review_id, $user, $page_size)
    {
        // dd($validated_data);
    ...
    public function showProductReviews($product_slug, $page_size)
    {
          $user = auth('sanctum')->check() ? auth('sanctum')->user() : null;
	...
	$percentages = collect([]);
// Step 1: Convert the second collection into an associative array
...
// Step 2 & 3: Iterate through the first collection and update the second collection
foreach ($percentages as $percentageItem) {


    // Extract the rating from the first collection item
    $ratingKey = $percentageItem['rating'];

    // Check if there is a corresponding entry in the second collection
    if (isset($secondCollectionArray[$ratingKey])) {
		
        // Update the second collection's item with the values from the first collection
...
// Optionally, convert the updated second collection back into a collection format
$updatedPercentages = collect(array_values($secondCollectionArray))->sortBy('rating')->values();
		
	

// Now, $updatedPercentages contains the merged and updated collection

		


	/*	$percentages = $ratings->each(function($item,$key) use($ratings,$product,$percentages){
$rating = 0;
			foreach($ratings->keys() as $key){
			

		
				
				if($item->first()['rating'] == $key){
				
				$rating =  $ratings->first()->where('rating', floatval((int)$key. ".0"))->first();
...
// Now, $percentages is a collection with 4 items, each being an array with 'rating', 'count', and 'percentage' keys.
...
//});
    ...
    public function show(int $review_id): Review
    {
        $review = Review::findOrFail($review_id);
    ...
    public function delete(int $review_id) : void
    {
        try {
            $review = Review::findOrFail($review_id);
    ...
    public function forceDelete(int $review_id) : void
    {
        $review = Review::findOrFail($review_id);
