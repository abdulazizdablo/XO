...
$orders = Order::query()->where(function ($query) {
    $query->where([['original_order_id', null],['status','!=', 'replaced']])
        ...
        ->orWhere(function ($query) {
            ...
            if ($type == 'open') {
                ...
                $orders = Order::whereIn('status', ['processing','in_delivery'])->where(function ($query) {
                    $query->where('paid', 1)
                        ->orWhere(function ($query) {
                            $query->where('paid', 0)
                                ->where('payment_method', 'cod');
    ...
    public function getOrderCards($employee, $inventory_id = null)
    {
        try {
            ...
            if ($employee->hasRole(Roles::WAREHOUSE_ADMIN)) {
...
//$user = User::findOrFail($user_id);
    ...
    public function createOrder($order_data, $user_id, $address_id, $branch_id, $city_id, $lat, $long)
    {
    ...
    public function createOrderItem($order_id, $order_items_data, $city)
    {
        $total_price = 0;
        $total_quantity = 0;
...
*/
        ...
        foreach ($grouped_product_variations as $key => $product_variations) {
            ...
            $stock = $this->checkStockLevel($product_variations, $product_variations_data, $city);
                ...
                switch ($key) {
                    case 'bogo':
                        ...
                        if (method_exists($this, $method)) {
                            ...
                            $total_quantity += $order_items_data['total_quantity'];
														Log::debug('bogo qty: '.$total_quantity);
							Log::debug('------------------------------');
                        }
                    ...
                    case 'btgo':
                        ...
                        if (method_exists($this, $method)) {
                            ...
                            $total_price += $order_items_data['price'];
                            $total_quantity += $order_items_data['total_quantity'];
														Log::debug('btgo qty: '.$total_quantity);
							Log::debug('------------------------------');
                        }
                    ...
                    case 'bogh':
                        ...
                        if (method_exists($this, $method)) {
                            ...
                            $total_price += $order_items_data['price'];
                            $total_quantity += $order_items_data['total_quantity'];
														Log::debug('bogh qty: '.$total_quantity);
							Log::debug('------------------------------');
                        }
                    ...
                    case 'discount':
                        if (method_exists($this, $method)) {
                            ...
                            $total_price += $order_items_data['price'];
                            $total_quantity += $order_items_data['total_quantity'];
														Log::debug('dis qty: '.$total_quantity);
							Log::debug('------------------------------');
                        }
                    ...
                    case 'no_group':
                        ...
                        if (method_exists($this, $method)) {
                            ...
                            $total_price += $order_items_data['price'];
                            $total_quantity += $order_items_data['total_quantity'];
														Log::debug('nog qty: '.$total_quantity);
							Log::debug('------------------------------');
                        }
    ...
    public function checkAvailableInCity($order_items_data, $city)
    {
        ...
        foreach ($order_items_data as $order_items) {
            ...
            $product_variation = ProductVariation::findOrFail($order_items['product_variation_id']);
...
public function checkAvailable($order_items_data)
    ...
    public function checkStockLevel($product_variations, $product_variation_data, $city)
    {
        ...
        foreach ($product_variations as $product_variation) {
            ...
            $stock_levels_in_city = $product_variation->stock_levels()->whereHas('inventory', function ($query) use ($city, $main_city) {
                $query->where('city_id', $city);
    ...
    public function calculateDiscount($product_variations, $product_variations_data, $order_id, $stock)
    {
        ...
        foreach ($product_variations as $product_variation) {
            ...
            $price = (($product_variation->product->pricing->value) * (100 - $percentage)) / 100;
    ...
    public function storeShippingInfo($shipping_info_data, $order_id, $address, $city_id, $city)
    {
        ...
        // if(isset($shipping_info_data['date'])){
        //     $date = Carbon::parse($shipping_info_data['date']);
        // }else{
    ...
    public function showOpenOrderItems($order_id, $employee){
        ...
        // }
            if($employee->hasRole(Roles::WAREHOUSE_ADMIN)||$employee->hasRole(Roles::WAREHOUSE_MANAGER)||$employee->hasRole(Roles::DELIVERY_ADMIN)||$employee->hasRole(Roles::MAIN_ADMIN)){
                ...
                $order_items = $order->order_items()
                    ->select('id', 'order_id', 'to_inventory', 'product_variation_id','status', 'quantity', 'price','on_hold')
                    ->with([
