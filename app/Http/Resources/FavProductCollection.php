...
class FavProductCollection extends ResourceCollection
{
    use TranslateFields;
    ...
    public function __construct($fav, $not,  $user = null, /*$favourites_products = null,*/ $notified_products = null, $user_reviews = null)
    {
        ...
        $this->fav = $this->collectResource($fav); //->with( $this->favourites_product);
        $this->not = $this->collectResource($not); //->with( $this->favourites_products);
    ...
    public function toArray($request)
    {
        ...
        if ($this->fav instanceof AbstractPaginator && $this->not instanceof AbstractPaginator) {
        ...
        $a= [
            'products' => $this->fav->transform(function ($item) use ($user) {
           ...
           $has_discount = isset($this->discount_id)? true : false;
				...
				Log::debug('$fav product_variations: '.$product_variations);
				Log::debug('========================================');
		...
		//$b= [
       return [
				'products' => $this->not->transform(function ($item) use ($user) {
                // Get pricing information for the product
                $pricing = $item->pricing;
           ...
           $has_discount = isset($this->discount_id)? true : false;
                ...
                // Get the product variations
                $product_variations = $item->product_variations;
				Log::debug('$not product_variations: '.$product_variations);
				Log::debug('========================================');
                $product_variations_collection = new ProductVariationCollection(
                    $product_variations,
                   // collect($this->notified_products)->pluck('product_variations.*.id') ?? null,
